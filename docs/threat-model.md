...
| Duplicate mutation through retries | Unregistered operations use no retries; replay decision precedes status policy | Unreviewed raw mutations remain an explicit high-risk escape hatch |
...
| Policy claims without tests | Build joins endpoint, practice, and test-evidence IDs and verifies functions exist | Function presence does not prove live-tenant behavior |
