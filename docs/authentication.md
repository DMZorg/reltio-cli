...
Reltio's default token lifetime is 60 minutes. Its [current token-rate guidance](https://docs.reltio.com/en/developer-resources/system-administration-apis/system-administration-apis-at-a-glance/authentication-api/authentication-token-api-rate-limiting) describes an undisclosed rolling limit shared by source IP. The CLI keeps a local ten-request-per-second throttle per authentication origin and cache directory; it cannot coordinate unrelated hosts or caches behind the same NAT, VPN, or proxy. Reuse tokens and coordinate shared-egress workloads through a secure shared cache or token service. After a `429`, reduce aggregate acquisition traffic and use increasing delays; contact Reltio Support for exceptional high-volume needs. Without Multi Token Support, an early token request may return the existing token with its original validity, so normal commands do not refresh speculatively. When explicit login receives the same token bytes already in cache, the CLI preserves the original acquisition time and caps the new expiry at the cached expiry instead of falsely extending that token's lifetime. If Reltio rejects a token and the provider reissues that same token, the CLI records that refresh generation and stops with `auth_token_unchanged`; concurrent and later processes do not create a repeated token-request/replay loop.

`auth status` reports `environment_override: true` and emits a warning when `RELTIO_CLIENT_ID`, `RELTIO_CLIENT_SECRET`, or `RELTIO_ACCESS_TOKEN` changes the selected profile credential source. `--quiet` suppresses that stderr narration but retains the warning in structured metadata. Login likewise retains the owner-only plaintext-cache warning in `meta.warnings` under `--quiet`.

Reltio's token-specific guidance says not to retry authentication token failures except rate-limit `429` responses. The CLI therefore makes one attempt for transport failures and non-`429` responses. It may replay a `429` once within the authentication deadline; `--no-retry` disables that replay as well as the one-time safe `401` refresh used for tenant API requests. An oversized `429` diagnostic body is omitted rather than allowed to suppress the replay. Because its uninspected provider bytes cannot be proven safe, the originating command receives a deny-all output guard even if the replay acquires a usable token; a later invocation may use the safely cached token normally.
...
### Profile routing and removal recovery

Changing a profile's environment, tenant, base URL, or service URLs requires `--clear-auth` whenever the profile contains authentication metadata. This includes a client ID or secret-file reference stored without an explicit method: environment credentials can activate those settings later. Set up authentication again for the new target.

If profile removal fails during cache cleanup and restores the profile, only its active imported bearer becomes usable again. Cleanup markers for older retired bearer generations remain queued so a later command can remove them. A restored profile without an active bearer retains all previously pending cleanup markers.
