...
Every finite command computes one absolute deadline from `--timeout`. Authentication acquisition, token-endpoint requests and bounded replay, credential-process execution, rate and maintenance locks, cache work, configuration locks, and the tenant request all consume the remaining portion of that same budget; no stage receives a fresh timeout. SIGINT is observed independently of command polling and cancels lock waits, process waits, retry sleeps, requests, and response reads. A cancellation check precedes each controlled local commit. If cancellation races with a commit point or a remote request, the error reports the observed local and remote state and is not converted into success.
