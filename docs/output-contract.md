...
Finite commands apply `--timeout` as one absolute wall-clock budget across target resolution, authentication, local locks and commits, request retries, and response-body reads. SIGINT is observed independently of command progress. Once observed, an otherwise successful result is replaced by `request_canceled`, commit and remote-completion fields are preserved where known, and the process exits `130`.
